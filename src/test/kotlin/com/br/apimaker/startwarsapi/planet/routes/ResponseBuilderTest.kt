...
class ResponseBuilderTest {
    ...
    fun `should return http created when a register was created`() {
        val response = responseBuilder.build(LoadPlanetDTO(created = true, output = createPlanetDTOOutput()))
    ...
    fun `should return http not found when planet do not exists`() {
        val response = responseBuilder.build(LoadPlanetDTO(created = false, output = null))
    ...
    fun `should return http ok when planet exists but was not created`() {
        val response = responseBuilder.build(LoadPlanetDTO(created = false, output = createPlanetDTOOutput()))
